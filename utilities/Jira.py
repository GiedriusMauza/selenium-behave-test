...
class Jira_utility( Reusable ):
    ...
    def connect_to_jira(context):
        ...
        if settings.use_proxy:
            jira = JIRA( options=jira_server, basic_auth=(settings.jira_user, settings.jira_password),
                         proxies={"http": "http://zscaler.test.com:80", "https": "https://zscaler.star.com.au:80"} )
        else:
            jira = JIRA( options=jira_server, basic_auth=(settings.jira_user, settings.jira_password) )
    ...
    def add_comment(context):
        ...
        jira.add_comment( issue,
                          'Automated Test Execution Reports available at Jenkins ->"http://localhost:8080/job/Patron/"' )
